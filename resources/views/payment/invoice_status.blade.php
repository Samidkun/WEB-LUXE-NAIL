            ...
            <div class="col-lg-8">
                <div class="card shadow-lg border-0 rounded-4 overflow-hidden">
                    <div class="card-header bg-white p-4 border-bottom text-center">
                        ...
                        <p class="text-muted mb-0">Queue Number: <span class="fw-bold text-dark">{{ $reservation->queue_number }}</span></p>
                    </div>
                    <div class="card-body p-5">
                        ...
                        <div class="text-center mb-5">
                            @if($reservation->status === 'cancelled')
                                ...
                                <p class="text-danger mt-2">This reservation has expired because payment was not completed within 1 hour.</p>
                        ...
                        <div class="row g-4 mb-5">
                            ...
                            <div class="col-md-6 text-md-end">
                                ...
                                <p class="mb-0 fw-bold">{{ \Carbon\Carbon::parse($reservation->reservation_date)->format('d M Y') }}</p>
                                <p class="mb-0 text-muted">{{ \Carbon\Carbon::parse($reservation->reservation_time)->format('H:i') }}</p>
                                <p class="mb-0 text-primary">{{ ucfirst(str_replace('_', ' ', $reservation->treatment_type)) }}</p>
                            </div>
                        </div>
                        ...
                        <div class="d-grid gap-2">
                            @if($reservation->status === 'pending')
                                <a href="{{ route('payment.show', $reservation->id) }}" class="btn btn-primary py-3 fw-bold" style="background: #d889a6; border:none;">
                                    Pay Now
                                </a>
                            ...
                            @else
                                <a href="{{ route('payment.invoice', $reservation->queue_number) }}" class="btn btn-outline-primary py-3 fw-bold">
                                    Download Invoice
                                </a>
                            @endif
